...
DialogGraph::DialogGraph(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogGraph), m_width(0), m_height(0),
      m_xMin(-3), m_xMax(20), m_x(0), m_scale(0), m_isError(0),
      m_num_of_grid(10), m_scene(nullptr) {
  ui->setupUi(this);
...
void DialogGraph::dataConversion() {
  ...
  while (!isfinite(m_res) && count < 1e8) {
    ...
    if (!isnormal((res0) && isnormal(res1)))
      res0 = 0;
    if (!isnormal(res1) && isnormal(res0))
      res1 = 0;
    m_res = (res0 + res1) / 2;
  }
...
void DialogGraph::createDialog() {
  ...
  if (!(m_scene = new QGraphicsScene(ui->graphicsView)))
    this->close();
